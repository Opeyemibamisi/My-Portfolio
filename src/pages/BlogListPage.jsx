  ...
  return (
    <main className="min-h-screen bg-mesh-dark pt-28">
      <section className="section-shell py-16">
        <SectionTitle
          eyebrow="Blog"
          title="Field notes for modern developers."
          body="Search by topic, explore categories, and open each post on its own reading page."
        />
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
            {categories.map((item) => (
              <button
                ...
                className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold transition ${category === item ? "bg-cyanx text-ink" : "glass text-[color:var(--muted)]"}`}
              >
                {item}
              </button>
            ))}
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {visible.map((post, index) => (
            ...
            >
              ...
              />
              <p className="mt-5 text-sm font-bold text-cyanx">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {post.tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full bg-white/8 px-3 py-1 text-xs font-bold text-[color:var(--muted)]"
                  >
                    #{tag}
                  </span>
                ))}
              ...
              <button
                ...
                className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-cyanx"
              >
